...
**<https://iloveflfl.github.io/minebeat-rush/>**

브라우저로 열면 됩니다. 설치 없음. 첫 로딩만 약 42MB(대부분 Godot 엔진 wasm)이고
이후는 캐시됩니다. 홈 화면에 추가하면 전체화면 앱처럼 뜹니다(PWA).

| 조작 | 폰 |
| --- | --- |
| 왼쪽 대시 | 화면 왼쪽 1/3 탭, 또는 왼쪽으로 플릭 |
| 앞으로 대시 | 화면 가운데 1/3 탭, 또는 위로 플릭 |
| 오른쪽 대시 | 화면 오른쪽 1/3 탭, 또는 오른쪽으로 플릭 |

터치 입력은 키보드와 **완전히 같은 액션**을 만들어 보내기 때문에, GDD 7.1 [LOCK]
(대각선 없음 / 코어 구간 후진 없음)이 그대로 적용됩니다.

### 새 버전 올리기

```bash
S:\GameDev\MineBeatRush\배포.bat patch
```

`patch` / `minor` / `major` 중 하나(생략하면 버전 유지). 스크립트가 순서대로
**테스트 실행 → 실패 시 배포 중단** → 웹 export → 커밋·태그 → `gh-pages` 브랜치 푸시를 합니다.
테스트가 하나라도 깨지면 배포되지 않습니다.
