...
func validate(start: Vector2i, max_dashes: int) -> PackedStringArray:
	...
	# GDD 27.1: no numbered clue may be hidden behind an obstacle.
	for r in length:
		for c in width:
			var cell := Vector2i(c, r)
			if state_at(cell) == Cell.OBSTACLE and number_at(cell) > 0:
				errors.append("obstacle at %s buries a clue of value %d" % [cell, number_at(cell)])
